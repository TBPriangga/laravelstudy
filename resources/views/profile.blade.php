...
<body class="bg-gray-900 text-white min-h-screen">
                ...
                @if (Auth::check())
                    <div class="relative">
                        ...
                        <div x-show="open" @click.outside="open = false" x-transition
                            class="absolute right-0 mt-3 bg-white dark:bg-gray-800 rounded-md shadow-lg w-40 border border-gray-700 z-50 origin-top-right">
                            <a href="{{ route('profile') }}"
                                class="block px-4 py-2 text-sm text-gray-200 hover:bg-gray-700 rounded-t-md">
                                Profile
                            </a>
                            <a href="{{ route('logout') }}"
                                class="block px-4 py-2 text-sm text-red-400 hover:bg-gray-700 rounded-b-md">
                                Logout
                            </a>
                        </div>
                    </div>
    ...
    {{-- Main Content --}}
    <main class="pt-24 px-6 lg:px-8">
        <div class="max-w-5xl mx-auto">
            {{-- Alert --}}
            @if (session('success'))
                <div class="mb-6 rounded-lg bg-green-100 text-green-800 p-4 shadow text-center">
                    {{ session('success') }}
                </div>
            @endif

            @if ($errors->any())
                <div class="mb-6 rounded-lg bg-red-100 text-red-800 p-4 shadow text-center">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                ...
                <div class="bg-gray-800 rounded-xl shadow-lg p-6 text-center">
                    ...
                    @if (Auth::user()->avatar)
                        <form action="{{ route('profile.deleteAvatar') }}" method="POST" class="mt-4">
                            ...
                            @method('DELETE')
                            <button type="submit"
                                class="text-sm text-red-400 hover:text-red-500 font-medium transition">Remove
                                Photo</button>
                        </form>
                    @endif
                </div>
                ...
                <div class="md:col-span-2 bg-gray-800 rounded-xl shadow-lg p-6">
                    ...
                    <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data"
                        ...
                        <div>
                            ...
                            <input type="file" name="avatar" id="avatar" accept="image/*"
                                ...
                                class="block w-full text-sm text-gray-200 border border-gray-600 rounded-lg bg-gray-900 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-semibold file:bg-indigo-50 file:text-indigo-600 hover:file:bg-indigo-100">
                        ...
                        <div>
                            ...
                            <input type="text" id="name" name="name" value="{{ old('name', Auth::user()->name) }}"
                                required
                                class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-indigo-500 focus:border-transparent">
                        </div>
                        ...
                        <div class="border-t border-gray-700 pt-6">
                            <h4 class="text-md font-semibold mb-4">Change Password</h4>
                            <p class="text-sm text-gray-400 mb-4">Leave blank if you don't want to change password</p>

                            <div class="space-y-4">
                                <input type="password" id="current_password" name="current_password"
                                    placeholder="Current Password"
                                    class="w-full px-4 py-2 bg-gray-900 border border-gray-700 rounded-lg text-white focus:ring-2 focus:ring-indigo-500">
                    ...
                    {{-- Delete Account --}}
                    <form action="{{ route('profile.deleteAccount') }}" method="POST" class="mt-8 text-center">
                        @csrf
                        @method('DELETE')
                        <button type="submit"
